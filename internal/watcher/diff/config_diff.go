...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.Codex.FingerprintConvergence != newCfg.Codex.FingerprintConvergence {
		changes = append(changes, fmt.Sprintf("codex.fingerprint-convergence: %s -> %s", displayOptionalValue(oldCfg.Codex.FingerprintConvergence), displayOptionalValue(newCfg.Codex.FingerprintConvergence)))
	}
